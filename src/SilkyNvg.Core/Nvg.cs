    ...
    public sealed class Nvg : IDisposable
    {
        ...
        public void Restore()
        {
                ...
                !ReferenceEquals(preClipSnapshot, postRestoreState.ClipSnapshot))
            {
                ...
                {
                    // Restored state has a clip — re-render it from the saved snapshot.
                    // First clear any existing clip mask, then rebuild from snapshot.
                    renderer.ClearClip();
                    renderer.SetClipFromSnapshot(
                        postRestoreState.ClipSnapshot.StencilVertices,
                        postRestoreState.ClipSnapshot.Bounds,
                        postRestoreState.ClipSnapshot.EvenOdd);
                }
                else
                {
